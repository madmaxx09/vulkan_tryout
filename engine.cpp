...
EngineDevice::EngineDevice(Window &window) : window{window} {
  createInstance();
  setupDebugMessenger(); //pour un débugage correct à enlever quand la performance est importante
  createSurface(); //connection entre glfw et vk
  pickPhysicalDevice(); //choppe le/les gpu (en géneral le)
  createLogicalDevice();
  createCommandPool();
}
